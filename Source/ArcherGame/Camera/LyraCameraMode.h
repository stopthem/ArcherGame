...
class ARCHERGAME_API ULyraCameraMode : public UObject
{
	GENERATED_BODY()
...
protected:
	...
	/**
	 * @brief Update camera view variables.
	 * @param DeltaTime 
	 * @param bOverrideCameraLag This overrides camera lag if there is one and sets the PivotLocation directly what CameraMode sends.
	 */
	virtual void UpdateView(float DeltaTime, bool bOverrideCameraLag = false);
...
public:
	// Do we want a lag on the camera?
	UPROPERTY(EditAnywhere, Category="Camera Lag")
	bool bDoCameraLag;

	// Camera lag interpolation speed.
	UPROPERTY(EditAnywhere, Category="Camera Lag", meta=(EditCondition="bDoCameraLag == true", UIMin="0", ClampMin="0"))
	float CameraLagSpeed;

	// Interpolation weight of the each axis. 0 means axis does not interpolate while 1 means axis interpolates.
	UPROPERTY(EditAnywhere, Category="Camera Lag", meta=(EditCondition="bDoCameraLag == true"))
	FVector CameraLagWeights;
