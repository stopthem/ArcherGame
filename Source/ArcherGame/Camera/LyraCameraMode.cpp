...
void FLyraCameraModeView::Blend(const FLyraCameraModeView& Other, float OtherWeight)
{
	...
	Location = UKismetMathLibrary::VLerp(Location, Other.Location, OtherWeight);
...
void ULyraCameraMode::OnActivation()
{
...
FRotator ULyraCameraMode::GetPivotRotation()
{
	const AActor* TargetActor = GetTargetActor();
	check(TargetActor);
...
void ULyraCameraMode::UpdateView(float DeltaTime, bool bOverrideCameraLag)
{
	const FVector PivotLocation = GetPivotLocation();
	const FRotator PivotRotation = GetPivotRotation();

	const FVector interpolatedLocation = UKismetMathLibrary::VInterpTo(View.Location, PivotLocation, GetWorld()->GetDeltaSeconds(), CameraLagSpeed);

	// We are lerping each axis from PivotLocation(uninterpolated location) to interpedLocation using CameraLagWeights.
	const FVector finalLocation = FVector(
		FMath::Lerp(PivotLocation.X, interpolatedLocation.X, CameraLagWeights.X),
		FMath::Lerp(PivotLocation.Y, interpolatedLocation.Y, CameraLagWeights.Y),
		FMath::Lerp(PivotLocation.Z, interpolatedLocation.Z, CameraLagWeights.Z));
